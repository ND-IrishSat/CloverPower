...
def main():
    if request.method == 'POST':
        ...
        if reg_info:
            ...
            description = interpret_register_data(reg_addr, data)
            binary_value = f'{data:02b}' if isinstance(data, int) else ' '.join([f'{byte:02b}' for byte in data])
            return render_template("index.html",registers=registers, reg_name=reg_info[0], binary_value=binary_value, description=description, allen = read_register_data(0x3D,2))
    else:
